...
@Component({
  selector: 'app-area-product-brand-wise',
  templateUrl: './area-product-brand-wise.component.html',
  styleUrl: './area-product-brand-wise.component.scss',
})
export class AreaProductBrandWiseComponent {
      ...
      this.productService.filterProducts().subscribe((response) => {
        ...
        if (this.status) {
          if (this.status === 'on-sale') {
            ...
            this.products = this.products.filter(
              (p) => p.status === 'out-of-stock' || p.quantity === 0
            );
          }
        }
        // brand filtering
  ...
  sortByFilter(value: string) {
    ...
    if (value === 'asc') {
      this.getProductsByBrand(); // Re-fetch original product list
    } else if (value === 'low') {
  ...
  setPage(page: number) {
    ...
    // this.baseProductSer.getProductsByBrand(this.brandID,this.pageNo-1).subscribe(res => {
  ...
  getProductsByBrand() {
    ...
    if (this.minPrice || this.maxPrice) {
      this.baseProductSer
        .getProductsByBrand(this.brand, this.pageNo,this.minPrice,this.maxPrice)
        .subscribe((res) => {
          this.productsList = res;
