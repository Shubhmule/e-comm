...
export class ShopAreaComponent {
  ...
  // product Pagination
  setPage(page: number) {
    this.productRouter.getshop(Number(this.brand),page-1,this.minPrice,this.maxPrice).subscribe(res=>
      {
...
// }
  ...
  getshopproduct(){   
    ...
    this.productRouter.getshop(this.brand,covertnum-1,this.minPrice,this.maxPrice).subscribe(res=>
    {
          this.productsList=res;
          this.paginate = this.productService.getPager(this.productsList[0]?.totalCount,Number(+this.pageNo),this.pageSize);
               this.isLoading=false
